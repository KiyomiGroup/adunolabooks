  ...
  return (
    <div style={{ maxWidth: "640px" }}>
      <div style={{ marginBottom: "2rem" }}>
        ...
        <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "2rem", fontWeight: 400, color: "var(--ink)", margin: "0.75rem 0 0" }}>{poem.title}</h1>
      </div>
      ...
      <form action={updateWithId} style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
        ...
        <StudioField label="Content" name="content" type="textarea" rows={14} required defaultValue={poem.content} hint="Write each line as its own line." />
        <StudioField label="Tags" name="tags" defaultValue={poem.tags?.join(", ") ?? ""} placeholder="grief, Lagos, home" />
        <StudioField label="Status" name="status" type="select" defaultValue={poem.status}
          options={[{ value: "draft", label: "Draft — not visible yet" }, { value: "published", label: "Published — live on site" }]} />
        ...
        <div style={{ display: "flex", gap: "1rem", paddingTop: "0.5rem" }}>
          ...
          <Link href="/admin/poems" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.85rem", color: "var(--muted)", textDecoration: "none", padding: "0.85rem 1.25rem", display: "inline-flex", alignItems: "center" }}>Cancel</Link>
        </div>
      </form>
    </div>
  );
